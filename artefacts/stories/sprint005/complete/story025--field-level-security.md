...
## PR Review - story-025: Configure Field-Level Security

Result: Approved

### Summary
- Verified all four target fields (EPA_StartDate__c, EPA_EndDate__c, EPA_HolidayDate__c, EPA_AnnualAllowanceDays__c) are defined with `<required>true</required>` in the local metadata — confirming Salesforce will auto-manage their visibility and editability based on object-level permissions. No explicit fieldPermissions entries are needed or allowed for these fields.
- Confirmed existing FLS in all three profiles and three permission sets correctly satisfies every acceptance criterion:
  - **AC1 (Apprentice):** Category editable, Status and CalculatedBusinessDays read-only, StartDate/EndDate auto-visible+editable via required field + object Edit access.
  - **AC2 (Line Manager):** All non-required fields read-only via FLS; StartDate/EndDate auto-visible but not editable because object-level allowEdit=false.
  - **AC3 (System Administrator):** Full read+edit on all fields via FLS + required field behaviour + full object CRUD.
  - **AC4 (Config objects):** LeaveCategory Description read-only for Apprentice/Manager; HolidayDate and AnnualAllowanceDays are required fields, auto-visible with object Read.
  - **AC5 (No excess visibility):** Each role only has FLS entries for fields relevant to their function.
- PR contains only the story file update with implementation record — no metadata changes. This is consistent with the finding and architecturally sound.
- Branch naming (`feature/025`), PR title (`story-025: ...`), and target branch (`Dev1`) comply with project conventions.
- No code quality issues — no code or metadata files were changed.
...
Target Org: sprint005

### Acceptance Criteria Results

- **AC1: Apprentice role FLS on Study Leave Request** — Pass
  - Given the Apprentice role, when FLS is inspected on EPA_StudyLeaveRequest__c, then:
    - View and edit Start Date, End Date: **Pass** — EPA_StartDate__c and EPA_EndDate__c are required fields (IsNillable=false), automatically visible and editable to any user with object-level Edit access.
    - View and edit Category: **Pass** — EPA_Category__c: PermissionsRead=true, PermissionsEdit=true.
    - View only Calculated Business Days: **Pass** — EPA_CalculatedBusinessDays__c: PermissionsRead=true, PermissionsEdit=false.
    - View only Status: **Pass** — EPA_Status__c: PermissionsRead=true, PermissionsEdit=false.
    - Not visible admin-only fields: **Pass** — No unexpected FLS entries found. Only 4 explicit fieldPermissions entries exist (Category, CalculatedBusinessDays, Status, Apprentice), plus 1 on LeaveCategory (Description). No admin-only fields are granted access.
  - Verification method: SOQL query on FieldPermissions (SobjectType='EPA_StudyLeaveRequest__c', Profile='EPA Apprentice') and FieldDefinition (IsNillable check on required fields).
  - Evidence: FieldPermissions query returned 4 records; FieldDefinition confirmed StartDate/EndDate are required (IsNillable=false).

- **AC2: Line Manager role FLS on Study Leave Request** — Pass
  - Given the Line Manager role, when FLS is inspected on EPA_StudyLeaveRequest__c, then:
    - View only Start Date, End Date: **Pass** — Required fields are auto-visible; object-level permissions prevent editing (allowEdit=false on profile).
    - View only Category: **Pass** — EPA_Category__c: PermissionsRead=true, PermissionsEdit=false.
    - View only Calculated Business Days: **Pass** — EPA_CalculatedBusinessDays__c: PermissionsRead=true, PermissionsEdit=false.
    - View only Status: **Pass** — EPA_Status__c: PermissionsRead=true, PermissionsEdit=false.
    - View only Apprentice Name: **Pass** — EPA_Apprentice__c: PermissionsRead=true, PermissionsEdit=false.
    - Not edit any request detail fields: **Pass** — All 4 explicit FLS entries have PermissionsEdit=false.
  - Verification method: SOQL query on FieldPermissions (SobjectType='EPA_StudyLeaveRequest__c', Profile='EPA Line Manager').
  - Evidence: FieldPermissions query returned 4 records, all with PermissionsEdit=false.

- **AC3: System Administrator FLS — full access** — Pass
  - Given the System Administrator role, when FLS is inspected, then administrators can view and edit all fields.
    - EPA_StudyLeaveRequest__c: All 4 explicit fields (Apprentice, CalculatedBusinessDays, Category, Status) have PermissionsRead=true, PermissionsEdit=true. Required fields (StartDate, EndDate) auto-accessible.
    - EPA_LeaveCategory__c: EPA_Description__c — PermissionsRead=true, PermissionsEdit=true.
    - EPA_PublicHoliday__c: EPA_HolidayDate__c — Required field, auto-accessible with full CRUD.
  - Verification method: SOQL query on FieldPermissions (Profile='EPA System Administrator') for all EPA custom objects.
  - Evidence: All FieldPermissions entries show PermissionsRead=true, PermissionsEdit=true.

- **AC4: Configuration objects FLS (Leave Category, Public Holiday, Study Leave Allowance)** — Pass
  - Given FLS is configured on configuration objects, when an apprentice or manager views these objects:
    - EPA_LeaveCategory__c.EPA_Description__c: Apprentice=read-only, Manager=read-only. **Pass.**
    - EPA_PublicHoliday__c.EPA_HolidayDate__c: Required field (IsNillable=false), auto-visible to all with object Read access. **Pass.**
    - EPA_StudyLeaveAllowance__mdt.EPA_AnnualAllowanceDays__c: Custom Metadata Type — readable by all users with appropriate access. Custom Metadata FLS is not enforced the same way as custom objects, which is a known platform behaviour. **Pass.**
  - Verification method: SOQL on FieldPermissions for config objects; FieldDefinition query confirming HolidayDate is required.
  - Evidence: LeaveCategory Description has correct read-only FLS for Apprentice and Manager profiles and permission sets. HolidayDate confirmed IsNillable=false.

- **AC5: No user role has excess visibility** — Pass
  - Given FLS is applied, when tested, no role has visibility into fields they do not require.
    - Apprentice: 5 EPA FLS entries (4 on StudyLeaveRequest + 1 on LeaveCategory) — all appropriate for function.
    - Manager: 5 EPA FLS entries (4 on StudyLeaveRequest + 1 on LeaveCategory) — all appropriate for function.
    - Admin: 5 EPA FLS entries (4 on StudyLeaveRequest + 1 on LeaveCategory) — full access as expected.
    - Permission sets mirror profile FLS exactly: Apprentice PS (5 entries), Manager PS (5 entries), Admin PS (5 entries).
  - Verification method: Full SOQL query of all FieldPermissions for EPA custom objects across all 3 profiles and 3 permission sets.
  - Evidence: No unexpected or excess FLS entries found. Each role only has FLS for fields relevant to their defined function.

### Summary
All 5 acceptance criteria passed. Field-level security is correctly configured across all three profiles (EPA Apprentice, EPA Line Manager, EPA System Administrator) and three permission sets (EPA_StudyLeaveApprentice_PermissionSet, EPA_StudyLeaveManager_PermissionSet, EPA_StudyLeaveAdministrator_PermissionSet). Required fields (StartDate, EndDate, HolidayDate, AnnualAllowanceDays) are auto-managed by Salesforce and do not require explicit FLS entries. The story is now complete.
