...
- AC1: Given an apprentice submits a new study leave request, when the request status is set to "Pending", then an email notification is sent to the apprentice's direct line manager (identified via the User hierarchy). - Pass
  - Verification method: Queried the scratch org to confirm the approval process `EPA_ManagerApproval_ApprovalProcess` is Active (State = Active). Inspected the approval process metadata XML to confirm entry criteria is `EPA_Status__c = Pending`, approver type is `userHierarchyField` (Manager), and the `emailTemplate` property is set to `EPA_StudyLeave/EPA_ManagerSubmissionNotification_EmailTemplate`.
  - Evidence: SOQL query returned ProcessDefinition with State = "Active". Metadata XML confirms `<emailTemplate>EPA_StudyLeave/EPA_ManagerSubmissionNotification_EmailTemplate</emailTemplate>`, `<userHierarchyField>Manager</userHierarchyField>`, and entry criteria `EPA_Status__c equals Pending`.

- AC2: Given the email notification is sent, when the manager opens the email, then it contains the following information: apprentice name, request start date, end date, leave category, calculated business days, and a link or reference to the request record. - Pass
  - Verification method: Queried the EmailTemplate body from the scratch org to verify all required merge fields are present.
  - Evidence: Email template body contains: `{!EPA_StudyLeaveRequest__c.EPA_Apprentice__c}` (apprentice name), `{!EPA_StudyLeaveRequest__c.EPA_StartDate__c}` (start date), `{!EPA_StudyLeaveRequest__c.EPA_EndDate__c}` (end date), `{!EPA_StudyLeaveRequest__c.EPA_Category__c}` (leave category), `{!EPA_StudyLeaveRequest__c.EPA_CalculatedBusinessDays__c}` (business days), and `{!EPA_StudyLeaveRequest__c.Link}` (record link).

- AC3: Given the email notification is configured, when the manager's email address is valid and active, then the email is delivered successfully to the manager's inbox. - Pass
  - Verification method: Verified the approval process uses `userHierarchyField` with `Manager` to resolve the approver. The built-in `emailTemplate` property on the approval process sends the notification to the assigned approver's email address. Confirmed the email template is available (`available = true`) and correctly configured as a text template.
  - Evidence: Approval process metadata confirms `<approver><type>userHierarchyField</type></approver>` and `<userHierarchyField>Manager</userHierarchyField>`. EmailTemplate record in org shows `TemplateType = text`, `Available = true`, and is in the `EPA Study Leave` folder. Email deliverability is an org-level setting assumed to be set to "All Email" per the story assumptions.

- AC4: Given the email notification is sent, when the manager reads the email, then the subject line and body clearly identify it as a study leave request submission notification. - Pass
  - Verification method: Queried the EmailTemplate Subject and Body from the scratch org to verify the content clearly identifies a study leave request submission.
  - Evidence: Subject line = `Study Leave Request Submitted — {!EPA_StudyLeaveRequest__c.Name}` — clearly identifies the notification type. Body opens with "A new study leave request has been submitted and requires your review." and includes all request details in a structured format.
