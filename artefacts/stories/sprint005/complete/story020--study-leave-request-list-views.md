...
- AC1: Given a user navigates to the Study Leave Request object tab, when they view the list views, then predefined list views are available — **Pass**
  - Verification method: SOQL query on ListView object filtering by SobjectType = 'EPA_StudyLeaveRequest__c' and DeveloperName LIKE 'EPA_%'.
  - Evidence: 4 list views returned: EPA_AllRequests ("All Requests"), EPA_ApprovedRequests ("Approved Requests"), EPA_MyRequests ("My Requests"), EPA_PendingRequests ("Pending Requests"). All 4 predefined views confirmed present in sprint005 org.

- AC2: Given a user selects a list view, when they apply filters, then they can filter by Status (Pending, Approved, Rejected, Cancelled) — **Pass**
  - Verification method: ListView describe API confirmed EPA_Status__c is a displayed column (picklist type) on all 4 list views. Standard Salesforce list view filter functionality supports filtering on any displayed column. PendingRequests and ApprovedRequests already pre-filter by EPA_Status__c.
  - Evidence: EPA_Status__c column present in all list view describe responses. PendingRequests WHERE clause: `EPA_Status__c = 'Pending'`. ApprovedRequests WHERE clause: `EPA_Status__c = 'Approved'`.

- AC3: Given a user selects a list view, when they apply filters, then they can filter by Apprentice (User lookup) — **Pass**
  - Verification method: ListView describe API confirmed EPA_Apprentice__r.Name is a displayed column (reference type) on all 4 list views. Standard list view filtering supports lookup field filtering. MyRequests pre-filters by EPA_Apprentice__r.Name = '$CurrentUser'.
  - Evidence: EPA_Apprentice__r.Name column present in all list view describe responses. MyRequests WHERE clause: `EPA_Apprentice__r.Name = '$CurrentUser'`.

- AC4: Given a user selects a list view, when they apply filters, then they can filter by date range (Start Date and/or End Date) — **Pass**
  - Verification method: ListView describe API confirmed EPA_StartDate__c and EPA_EndDate__c are displayed columns (date type) on all 4 list views. Standard Salesforce list view filtering supports date field filtering with range operators.
  - Evidence: EPA_StartDate__c and EPA_EndDate__c columns present in all list view describe responses with type "date" and sortable=true.

- AC5: Given a user selects a list view, when they apply filters, then they can filter by Leave Category — **Pass**
  - Verification method: ListView describe API confirmed EPA_Category__r.Name is a displayed column (reference type) on all 4 list views. Standard Salesforce list view filtering supports lookup field filtering.
  - Evidence: EPA_Category__r.Name column present in all list view describe responses with type "reference".

- AC6: Given the list views are configured, when a user views a list, then the columns displayed include: Request Name, Apprentice, Start Date, End Date, Category, Calculated Business Days, and Status — **Pass**
  - Verification method: ListView describe API for all 4 list views verified the visible columns. Cross-referenced metadata XML files.
  - Evidence: All 4 list views display identical column set: Name (Study Leave Request Number), EPA_Apprentice__r.Name (Apprentice), EPA_StartDate__c (Start Date), EPA_EndDate__c (End Date), EPA_Category__r.Name (Category), EPA_CalculatedBusinessDays__c (Calculated Business Days), EPA_Status__c (Status). Hidden system columns (Id, CreatedDate, LastModifiedDate, SystemModstamp) are also present but not user-visible.

- AC7: Given record-level security is enforced, when a user views a list, then they only see records they have access to per the sharing model — **Pass**
  - Verification method: Tooling API query on EntityDefinition to verify OWD sharing model for EPA_StudyLeaveRequest__c. All list views use filterScope "Everything" which respects the object's sharing model — users only see records they have access to.
  - Evidence: InternalSharingModel = "Private", ExternalSharingModel = "Private". The Private sharing model ensures users only see records they own or have been granted access to via sharing rules, role hierarchy, or manual sharing. List views do not override record-level security.

### Summary
All 7 acceptance criteria passed. The 4 predefined list views (All Requests, My Requests, Pending Requests, Approved Requests) are correctly deployed to the sprint005 scratch org with the required columns, filters, and shared visibility. Record-level security is enforced by the object's Private sharing model. The story is now completed.
