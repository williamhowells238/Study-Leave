...
## PR Review - story-015: Add in-app notification to manager on study leave request submission

Result: Approved

### Summary
- **Naming conventions**: All components follow the `EPA_CamelCaseName_Suffix` convention. Git branch (`feature/015`) and PR title (`story-015:`) follow project standards.
- **Flow type selection**: After-save Record-Triggered Flow is the correct choice — the flow sends a custom notification (side effect), which cannot be done in a before-save context.
- **Bulk safety**: Linear flow with no loops. Two Get Records elements and one Send Custom Notification action execute sequentially outside any loop — no governor limit risk.
- **Fault handling**: Fault connector on the Send Custom Notification action routes to `Handle_Notification_Error`, capturing `$Flow.FaultMessage`. Consistent with the existing error handling pattern in `EPA_ManagerApprovalSubmit_Flow`.
- **Automation density**: Second after-save flow on `EPA_StudyLeaveRequest__c` (Create, Status=Pending) alongside `EPA_ManagerApprovalSubmit_Flow`. Both are independent fire-and-forget actions with no recursion risk or triggering record updates.
- **Acceptance criteria**: All four ACs fully covered — notification sent to manager on Pending status, message identifies apprentice by name, clicking notification navigates to the request record, and a dedicated Custom Notification Type exists with Desktop and Mobile channels.
- **Custom Notification Type**: Correctly queries by DeveloperName at runtime for cross-org portability. Desktop and Mobile channels enabled.
- **Decision element**: Handles null ManagerId gracefully, ending the flow without error if no manager is assigned.
