...
- AC1: Given the Scratch Org is available, when profiles are configured, then distinct profiles exist for Apprentice, Line Manager, and System Administrator — **Pass**
  - Verification method: Queried `Profile` object in sprint002 scratch org using SOQL: `SELECT Id, Name FROM Profile WHERE Name LIKE 'EPA%'`.
  - Evidence: 3 profiles returned — EPA Apprentice (00eC1000003U3INIA0), EPA Line Manager (00eC1000003U3IOIA0), EPA System Administrator (00eC1000003U3IPIA0).

- AC2: Given the Apprentice profile is configured, when an apprentice user logs in, then they have access to the Study Leave Request tab and can create, view, and edit their own Study Leave Request records — **Pass**
  - Verification method: Queried `ObjectPermissions` for EPA Apprentice profile; reviewed profile metadata XML for tab visibility and field-level security.
  - Evidence: EPA_StudyLeaveRequest__c — Read=true, Create=true, Edit=true, Delete=false. EPA_LeaveCategory__c — Read=true only. No access to EPA_PublicHoliday__c or EPA_StudyLeaveAllowance__mdt (0 records returned). Tab visibility: EPA_StudyLeaveRequest__c=DefaultOn, EPA_LeaveCategory__c=Hidden, EPA_PublicHoliday__c=Hidden. FLS: EPA_Category__c editable, EPA_Status__c/EPA_CalculatedBusinessDays__c/EPA_Apprentice__c read-only. Note: "own records only" filtering is controlled by OWD/sharing rules (Story 026), not profile-level permissions.

- AC3: Given the Line Manager profile is configured, when a manager user logs in, then they have access to the Study Leave Request tab and can view Study Leave Requests from their direct reports AND access the approval functionality — **Pass**
  - Verification method: Queried `ObjectPermissions` for EPA Line Manager profile; reviewed profile metadata XML for tab visibility and field-level security.
  - Evidence: EPA_StudyLeaveRequest__c — Read=true, Create=false, Edit=false, Delete=false. EPA_LeaveCategory__c — Read=true only. No access to EPA_PublicHoliday__c or EPA_StudyLeaveAllowance__mdt (0 records returned). Tab visibility: EPA_StudyLeaveRequest__c=DefaultOn, EPA_LeaveCategory__c=Hidden, EPA_PublicHoliday__c=Hidden. All FLS fields read-only. Note: "direct reports" record visibility depends on OWD/sharing rules (Story 026); approval functionality access depends on approval process configuration (Story 013). Profile baseline is correctly configured.

- AC4: Given the System Administrator profile is configured, when an admin user logs in, then they have full CRUD access to all custom objects — **Pass**
  - Verification method: Queried `ObjectPermissions` for EPA System Administrator profile; reviewed profile metadata XML for tab visibility and field-level security.
  - Evidence: EPA_StudyLeaveRequest__c — full CRUD + ModifyAllRecords + ViewAllRecords. EPA_LeaveCategory__c — full CRUD + ModifyAllRecords + ViewAllRecords. EPA_PublicHoliday__c — full CRUD + ModifyAllRecords + ViewAllRecords. All tabs (EPA_StudyLeaveRequest__c, EPA_LeaveCategory__c, EPA_PublicHoliday__c) set to DefaultOn. All FLS fields editable and readable.

- AC5: Given profiles are applied to users, when users access the application, then their experience is limited to the access granted by their profile — **Pass**
  - Verification method: Compared object permissions, tab visibility, and field-level security across all three profiles to verify differentiated access.
  - Evidence: Apprentice has restricted Create/Read/Edit (no Delete) on StudyLeaveRequest only. Line Manager has Read-only on StudyLeaveRequest only. System Administrator has full CRUD on all objects. Tab visibility is appropriately restricted per role. Negative tests confirmed: Apprentice and Line Manager have zero access to EPA_PublicHoliday__c and EPA_StudyLeaveAllowance__mdt.

### Negative Test Results

- Apprentice cannot delete Study Leave Requests — **Pass** (Delete=false confirmed)
- Apprentice has no access to EPA_PublicHoliday__c — **Pass** (0 ObjectPermissions records)
- Apprentice has no access to EPA_StudyLeaveAllowance__mdt — **Pass** (0 ObjectPermissions records)
- Line Manager cannot create/edit/delete Study Leave Requests — **Pass** (Create=false, Edit=false, Delete=false confirmed)
- Line Manager has no access to EPA_PublicHoliday__c — **Pass** (0 ObjectPermissions records)
- Line Manager has no access to EPA_StudyLeaveAllowance__mdt — **Pass** (0 ObjectPermissions records)
- Apprentice cannot see Leave Category or Public Holiday tabs — **Pass** (visibility=Hidden confirmed in metadata)
- Line Manager cannot see Leave Category or Public Holiday tabs — **Pass** (visibility=Hidden confirmed in metadata)

### Summary
All tests passed. All 5 acceptance criteria verified with positive and negative testing. The story is now complete.
