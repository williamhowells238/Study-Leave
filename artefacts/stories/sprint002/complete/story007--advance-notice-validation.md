...
Scratch Org: sprint002

### Acceptance Criteria Results

- **AC1:** Given an apprentice submits a study leave request with a start date less than one month from today, when the record is saved, then the system rejects the request with the error message — **Pass**
  - Verification method: Created an `EPA_StudyLeaveRequest__c` record with `EPA_StartDate__c=2026-04-22` (tomorrow, less than 1 month away) via `sf data create record`.
  - Evidence: Record creation failed with error: `"Study leave requests must be submitted at least one month before the start date."` — matches AC1 error message verbatim.

- **AC2:** Given an apprentice submits a study leave request with a start date one month or more from today, when the record is saved, then the request is accepted — **Pass**
  - Verification method: Created an `EPA_StudyLeaveRequest__c` record with `EPA_StartDate__c=2026-06-22` (2 months away) via `sf data create record`. Also tested the boundary case with `EPA_StartDate__c=2026-05-21` (exactly 1 month from today).
  - Evidence: Both records created successfully (IDs: `a02C1000007p8ZBIAY` and `a02C1000007pE1iIAE`). No validation error was raised. Boundary case (exactly 1 month) correctly accepted, confirming `ADDMONTHS(TODAY(), 1)` comparison behaviour.

- **AC3:** Given an apprentice edits an existing request and changes the start date to less than one month from today, when the record is saved, then the validation rejects the edit with the same error message — **Pass**
  - Verification method: Updated existing record `a02C1000007p8ZBIAY` (originally start date 2026-06-22) to `EPA_StartDate__c=2026-04-25` via `sf data update record`.
  - Evidence: Update failed with error: `"Study leave requests must be submitted at least one month before the start date."` — same error message as AC1, confirming validation fires on both insert and update.

### Apex Test Results
- All 12 Apex tests passed (100% pass rate, 0 failures).
- Test Run ID: `707C100000HbHxI`
- No regressions detected.

### Summary
All tests passed. All three acceptance criteria verified in the scratch org with correct validation behaviour on insert, boundary, and update scenarios. The story is now complete.
