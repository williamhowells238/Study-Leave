...
- AC1: Given an apprentice is logged in, when they navigate to the Study Leave Request area, then they see an option to create a new study leave request - **Pass**
  - Verification method: Verified the Page Layout (`EPA_StudyLeaveRequest__c-Study Leave Request Layout`) and FlexiPage (`EPA_StudyLeaveRequest_FlexiPage`) are deployed. Successfully created a new Study Leave Request record via the org, confirming the creation interface is available.
  - Evidence: Record `SLR-0001` (Id: a02C1000007pFU1IAM) created successfully. Page Layout source confirmed with Request Details and Record Information sections.

- AC2: Given the apprentice is on the new request form, when they view the form, then they can select a Start Date, End Date, and Leave Category (Exam Prep, Classroom Training, Revision) - **Pass**
  - Verification method: Created three Leave Category records (Exam Prep, Classroom Training, Revision). Created a Study Leave Request with Start Date, End Date, and Category lookup populated.
  - Evidence: Record created with EPA_StartDate__c=2026-06-01, EPA_EndDate__c=2026-06-05, EPA_Category__c=a00C100000GQOdNIAX (Exam Prep). All three category options available.

- AC3: Given the apprentice has filled in all required fields, when they submit the form, then a new record is created with the apprentice automatically set as the requesting user AND the status is set to "Pending" - **Pass**
  - Verification method: Created a record and queried EPA_Status__c and EPA_Apprentice__c fields. Compared EPA_Apprentice__c against CreatedById to confirm auto-population by the Before-Save Flow.
  - Evidence: EPA_Status__c = "Pending". EPA_Apprentice__c = 005C1000006ZM0bIAG matches CreatedById = 005C1000006ZM0bIAG. Flow `EPA_StudyLeaveSubmission_Flow` confirmed active and correctly assigns $User.Id and "Pending".

- AC4: Given the apprentice has not filled in all required fields, when they attempt to submit the form, then the form displays appropriate required-field error messages AND the record is not created - **Pass**
  - Verification method: Attempted to create a record without the EPA_Category__c field to trigger the validation rule.
  - Evidence: Record creation failed with error: "Category is required. Please select a leave category for this Study Leave Request." Validation rule `EPA_CategoryRequired_ValidationRule` fired correctly. No record was created.

- AC5: Given the form is submitted successfully, when the apprentice views the confirmation, then they can see the details of the submitted request including the calculated business days - **Pass**
  - Verification method: Queried EPA_CalculatedBusinessDays__c on two records: (1) Mon-Fri range (5 days expected), (2) two-week range spanning weekends (10 days expected). Verified the FlexiPage includes Record Detail component for confirmation view.
  - Evidence: Record 1 (Jun 1-5): EPA_CalculatedBusinessDays__c = 5 (correct, Mon-Fri). Record 2 (Jun 1-12): EPA_CalculatedBusinessDays__c = 10 (correct, 10 weekdays excluding 2 weekends). FlexiPage `EPA_StudyLeaveRequest_FlexiPage` deployed with Highlights Panel and Detail Panel.

### Apex Test Results
- All 12 Apex tests passed at 100% pass rate (Test Run Id: 707C100000HbGjj).
